...
| 🌊 **Clean Water Equivalent** | ₹1 lakh protected ≈ 50,000 litres of clean water infrastructure ([WHO/UNICEF JMP](https://washdata.org) cost benchmarks for rural India) |
| 🌳 **Trees Equivalent** | ₹500 protected ≈ 1 tree planted equivalent ([Trillion Tree Campaign](https://trilliontrees.org) avg sapling cost in India: ₹400–600) |
| 🏭 **CO₂ Offset** | ₹1,000 protected ≈ 2.5 kg CO₂ offset ([Gold Standard](https://www.goldstandard.org) voluntary carbon market avg ≈ $5/tonne CO₂) |
...
|--------|-------------|-----------------|
| **Alpha Vantage** | Real-time stock & forex quotes (AAPL, MSFT, GOOGL…) — price movements converted to transactions | Free key at [alphavantage.co](https://www.alphavantage.co) |
...
**Multi-stage build** — builder stage compiles native wheels, runtime stage is slim (~200MB).
**Persistent volumes** — alert data + Pathway state snapshots survive container restarts.
**Health check** — built-in `/api/health` endpoint polled every 30s.
...
- **Hybrid Architecture**: Live Market APIs → Pathway streams → ML scores → Quantum classifies → LLM explains → Automated Reports
- **Production-grade Simulator**: Custom numpy quantum engine implements all gates as proper unitary matrices — same math as real hardware
- **Explainable AI**: Every decision is traceable (ML features → quantum states → LLM reasoning → risk factor decomposition)
